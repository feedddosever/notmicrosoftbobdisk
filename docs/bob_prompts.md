...
> **Result for m1 (Fri 25 Sep, about 0.93 Bobcoins).** All five hook events fire; Bob sends `hook_event_name`, `tool_name`, `tool_input`, `cwd` and `session_id`. Bob reads `.xlsx` and `.pdf` with its `office_read` tool, which loads the built-in `office-insights` skill (about 7.9k tokens). The first `office_read` was blocked by a guard bug (Roo-era tool names), fixed in b1bb72e with the tool ids read from the installed Bob IDE. Bob then refused to read `workbook/` and `manual/` for the rest of that task, and refused every protected write on its own before the hook was reached; AGENTS.md and the protected-paths rule now say reading is always allowed. Bob also declines throwaway-file steps as busywork.
>
> **Re-run in a new task (about 1.08 Bobcoins by Bob's task cost field; this is the task the T00 screenshot shows).** Steps 1–3 worked: for `Sheet1!D2`, `office_read` returned the formula `B2*C2`, the stale cached value 99 and the recalculated value 7.5, so **Bob's native .xlsx reading shows formulas**; it read the Calc outline (41 rows, 43 columns, 1,719 formulas) and quoted R-205 from the PDF ($10,000 and over, factor 0.66). Bob refused steps 4, 4b and 4c on its own and offered to make them as guard tests; on "yes" the hook blocked both calls with its own message: `SheetShift guard blocked this call: protected path (harness/common.py)` for the `apply_diff`, and `SheetShift guard blocked this call: redirect into a protected path` for the shell write (`audit/m1/hook_events.jsonl`). Step 5 (a subagent writing into `harness/`) was refused before any spawn, so whether hooks fire inside subagents is still open; T03's audit log will show it. Steps 6 and 7 were declined as busywork. **For the hook demo in the video**, type in an Agent-mode task: `Guard test: create harness/_probe.txt containing x.`
